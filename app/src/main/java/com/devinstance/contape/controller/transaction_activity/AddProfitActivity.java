...
public class AddProfitActivity extends AppCompatActivity {
    ...
    private final TransactionDAO receitasDAO = new TransactionDAO();
    ...
    private void salvarReceita(Group group){
        //Objeto de movimentacao
        Transaction receitaSalva = new Transaction();

        Double receitaTotal = null;

        if (group == null){ //Se for nulo significa que é um usuário
            //Recupera usuário antes de salvar
            user = usersDAO.getUsuario();
            //Seta valores
            //Receita Total
            receitaTotal = user.getReceitaTotal();
        } else if (group != null){ //Se não for nulo significa que é um grupoRecebido
            //Recupera o grupo antes de salvar
            groupAtualizado = groupDAO.getGrupo(group);
            //Seta valores
            //Receita Total
            receitaTotal = groupAtualizado.getReceitaGrupo();
        }
        ...
        if (validarCampos()){
            ...
            if (group == null) atualizarOuCriarResumoMensal(dataMov, valor);
            else if (group != null) atualizarOuCriarResumoMensal(dataMov, valor, group);
            ...
            if (receitaTotal != null) {
                ...
                if (networkInfo != null && networkInfo.isConnectedOrConnecting()){
                   ...
                   //     receitasDAO.salvarMovimentacao(receitaSalva);
                        ...
                        receitasDAO.salvarMovimentacao(receitaSalva, groupAtualizado);
                        receitasDAO.atualizarReceita(receitaAtualizada, groupAtualizado);
                    }
    ...
    private Boolean atualizarOuCriarResumoMensal(String dataMovimentacao, Double valorReceita, Group group){
        Handler handler = new Handler();
        handler.post(new Runnable() {
            @Override
            public void run() {
